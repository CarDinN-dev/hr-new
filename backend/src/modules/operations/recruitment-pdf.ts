...
export function interviewAssessmentPdf(candidate: CandidateDocumentData) {
  ...
  ratingWidths.forEach((width, index) => { cell(doc, rx, y, width, 28, index === 0 ? [169, 208, 142] : index === 1 ? [151, 173, 198] : index === 2 ? [248, 224, 212] : index === 3 ? [226, 239, 216] : [232, 232, 232]); cellText(doc, ratingLabels[index], rx, y, width, 28, 5.7, false, 'center'); rx += width; }); y += 34;
  ...
  for (const [description, rating, remarks, height] of evaluation) { cell(doc, x, y, descriptionWidth, height); cellText(doc, description, x, y, descriptionWidth, height, 5.35); for (let score = 0; score < 5; score += 1) cell(doc, x + descriptionWidth + score * scoreWidth, y, scoreWidth, height); cell(doc, x + descriptionWidth + scoreWidth * 5, y, remarksWidth, height); ratingsMark(doc, rating, x + descriptionWidth + (5 - Number(rating || 0)) * scoreWidth, y + height / 2 + 3, scoreWidth); cellText(doc, remarks, x + descriptionWidth + scoreWidth * 5, y, remarksWidth, height, 5.7); y += height; }
