...
export class AuditController {
  ...
  @Permissions('audit.configure') @Get('policy') policy(@CurrentUser() user: RequestUser) { return this.audit.policy(user); }
  @Permissions('audit.configure') @Post('policy') updatePolicy(@Body() dto: UpdateAuditPolicyDto, @CurrentUser() user: RequestUser) { return this.audit.updatePolicy(dto, user); }
  @Permissions('audit.configure') @Get('legal-holds') holds(@CurrentUser() user: RequestUser) { return this.audit.listLegalHolds(user); }
  @Permissions('audit.configure') @Post('legal-holds') createHold(@Body() dto: CreateLegalHoldDto, @CurrentUser() user: RequestUser) { return this.audit.createLegalHold(dto, user); }
  @Permissions('audit.configure') @Post('legal-holds/:id/release') releaseHold(@Param('id') id: string, @Body() dto: ReleaseLegalHoldDto, @CurrentUser() user: RequestUser) { return this.audit.releaseLegalHold(id, dto, user); }
