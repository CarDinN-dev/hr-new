...
test("Admin can explore and export the department role hierarchy without changing the organization chart", async ({ page }) => {
  ...
  const rolePanel = page.locator("#role-hierarchy-panel");
  const roleHierarchy = page.getByRole("group", { name: "Company role hierarchy" });
  await expect(roleHierarchy).toBeVisible();
  await expect(rolePanel).toHaveCSS("padding", "0px");
  ...
  const [rolePanelBox, roleHierarchyBox, flowViewportBox] = await Promise.all([rolePanel.boundingBox(), roleHierarchy.boundingBox(), flowViewport.boundingBox()]);
  expect(Math.abs(roleHierarchyBox!.x - rolePanelBox!.x)).toBeLessThanOrEqual(2);
  expect(Math.abs(roleHierarchyBox!.x + roleHierarchyBox!.width - rolePanelBox!.x - rolePanelBox!.width)).toBeLessThanOrEqual(2);
  expect(Math.abs(flowViewportBox!.x - rolePanelBox!.x)).toBeLessThanOrEqual(2);
  expect(Math.abs(flowViewportBox!.x + flowViewportBox!.width - rolePanelBox!.x - rolePanelBox!.width)).toBeLessThanOrEqual(2);
  expect(Math.abs(flowViewportBox!.y + flowViewportBox!.height - rolePanelBox!.y - rolePanelBox!.height)).toBeLessThanOrEqual(2);
  await expect(flowViewport).toHaveAttribute("aria-describedby", "company-role-canvas-help");
  const enterFullscreen = page.getByRole("button", { name: "Enter fullscreen hierarchy" });
  await expect(enterFullscreen).toBeEnabled();
  await enterFullscreen.click();
  await expect.poll(() => flowViewport.evaluate(element => document.fullscreenElement === element)).toBe(true);
  await expect(page.getByRole("button", { name: "Exit fullscreen hierarchy" })).toHaveAttribute("aria-pressed", "true");
  const fullscreenBox = (await flowViewport.boundingBox())!;
  expect(fullscreenBox.x).toBeLessThanOrEqual(1);
  expect(fullscreenBox.y).toBeLessThanOrEqual(1);
  expect(Math.abs(fullscreenBox.width - page.viewportSize()!.width)).toBeLessThanOrEqual(1);
  expect(Math.abs(fullscreenBox.height - page.viewportSize()!.height)).toBeLessThanOrEqual(1);
  await page.getByRole("button", { name: "Exit fullscreen hierarchy" }).click();
  ...
  await expect(page.getByText("6 matching people")).toBeVisible();
  await expect(page.getByRole("group", { name: "Search result navigation" })).toContainText("1 of 6");
  await page.getByRole("button", { name: "Next role hierarchy result" }).click();
  await expect(page.getByRole("group", { name: "Search result navigation" })).toContainText("2 of 6");
  await search.fill("");
...
test("Sidebar surfaces and topbar account menu follow the selected theme", async ({ page }) => {
  ...
  await expect.poll(() => color(".logo-crop.wordmark", "backgroundColor")).toBe("rgb(247, 248, 252)");
