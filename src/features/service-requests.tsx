...
export function ServiceRequestsPanel({ session, notify }: { session: BackendSession; notify: (message: string) => void }) {
  ...
  const focusedRequestId = useHashRecordId("service-request");
  const focusedRequest = useQuery({ queryKey: [...workflowKey(session, "service-request-focus"), focusedRequestId], queryFn: () => apiRequest<ServiceRequest>(`/service-requests/${focusedRequestId}`), enabled: canRead && Boolean(focusedRequestId), retry: false });
  const visibleRequests = [...(focusedRequest.data && !requests.data?.data.some(item => item.id === focusedRequest.data.id) ? [focusedRequest.data] : []), ...(requests.data?.data ?? [])];
  useDeepLinkFocus("service-request", focusedRequestId, !focusedRequestId || focusedRequest.isSuccess || focusedRequest.isError, visibleRequests.some(item => item.id === focusedRequestId), notify);
  ...
  const create = useMutation({
    ...
    onSuccess: async () => { await refresh(); setConfirmCreate(false); notify("Certificate request submitted."); },
  });
  const transition = useMutation({
    mutationFn: async (dialog: ActionDialog) => {
      if (requiresStepUp && dialog.action === "override" && session.authProvider === "local") {
  ...
  return <section className="panel span-2">
    <div className="panel-head"><div><h3>Certificates</h3><span>Salary, experience and clearance requests.</span></div>{Boolean(session.employeeId) && hasPermission(session, "service_request.self.create") && <div className="inline-controls"><select aria-label="Certificate type" value={requestType} onChange={event => setRequestType(event.target.value)}><option value="SALARY_CERTIFICATE">Salary certificate</option><option value="EXPERIENCE_CERTIFICATE">Experience certificate</option><option value="CLEARANCE_CERTIFICATE">Clearance certificate</option></select><button className="primary" disabled={create.isPending} onClick={() => setConfirmCreate(true)}>Request</button></div>}</div>
    {requests.isPending ? <p className="muted">Loading certificate requests…</p> : requests.isError ? <p className="sync-alert">{requests.error.message}</p> : <><div className="list-stack">{visibleRequests.map(request => <div id={`service-request-${request.id}`} tabIndex={-1} className="workflow-card" key={request.id}><div className="list-row"><div><strong>{displayTitle(request.requestType)} · {request.subject.firstName} {request.subject.lastName}</strong><span>{displayTitle(request.status)} · {displayDate(request.createdAt)} · version {request.version}</span></div><div className="row-actions"><button onClick={() => setExpandedId(previous => previous === request.id ? null : request.id)}>{expandedId === request.id ? "Hide history" : "History"}</button>{request.status === "PUBLISHED" && hasAnyPermission(session, "service_request.self.download", "service_request.pdf.download_all") && <button onClick={() => void download(request.id).catch(error => notify(error.message))}><FileCheck2 size={15} /> Download</button>}<details className="card-actions-menu"><summary aria-label={`Actions for ${displayTitle(request.requestType)} requested by ${request.subject.firstName} ${request.subject.lastName}`}>Actions</summary><div className="card-actions-menu__items">{request.status === "SUBMITTED" && hasPermission(session, "service_request.hr.generate") && <button onClick={() => open(request, "review")}>Start review</button>}{request.status === "IN_HR_REVIEW" && hasPermission(session, "service_request.hr.generate") && <button onClick={() => open(request, "generate")}>Generate</button>}{request.status === "GENERATED" && hasPermission(session, "service_request.hr.generate") && <><button onClick={() => open(request, "generate")}>Regenerate</button><button onClick={() => open(request, "submit-approval")}>Send for approval</button></>}{request.status === "PENDING_HR_APPROVAL" && (hasPermission(session, "service_request.hr.approve") || hasActiveSuperAdminRole(session)) && <button onClick={() => open(request, "approve")}>Approve</button>}{!["REJECTED", "CANCELLED", "REVOKED", "PUBLISHED"].includes(request.status) && hasPermission(session, "service_request.hr.reject") && <button className="danger-outline" onClick={() => open(request, "reject", true)}>Reject</button>}{request.status === "APPROVED" && hasPermission(session, "service_request.hr.publish") && <button onClick={() => open(request, "publish")}>Publish</button>}{request.status === "PUBLISHED" && hasPermission(session, "service_request.hr.revoke") && <button className="danger-outline" onClick={() => open(request, "revoke", true)}>Revoke</button>}{!["PUBLISHED", "REJECTED", "CANCELLED", "REVOKED"].includes(request.status) && request.requesterUserId === session.id && hasPermission(session, "service_request.self.cancel") && <button onClick={() => open(request, "cancel", true)}>Cancel</button>}{hasPermission(session, "service_request.override") && <button onClick={() => open(request, "override", true)}>Override</button>}</div></details></div></div>{expandedId === request.id && <div className="workflow-history">{request.events?.map(event => <div key={event.id}><strong>{displayTitle(event.type)}</strong><span>{event.actor?.email || "System"} · {new Date(event.createdAt).toLocaleString()}{event.reason ? ` · ${event.reason}` : ""}</span></div>)}{!request.events?.length && <span className="muted">No events recorded.</span>}</div>}</div>)}{!visibleRequests.length && <div className="empty compact">No certificate requests.</div>}</div><Pagination total={requests.data?.meta?.total ?? 0} page={requests.data?.meta?.page ?? page} limit={requests.data?.meta?.limit ?? 15} totalPages={requests.data?.meta?.totalPages ?? 1} label="requests" loading={requests.isFetching} onPage={setPage} /></>}
    {create.isError && <p className="sync-alert">{create.error.message}</p>}
    {confirmCreate && <Dialog title={`Request ${displayTitle(requestType)}`} onClose={() => setConfirmCreate(false)}><p>This will send your certificate request to HR for review.</p><div className="modal-actions"><button onClick={() => setConfirmCreate(false)}>Cancel</button><button className="primary" disabled={create.isPending} onClick={() => create.mutate()}>Submit request</button></div>{create.isError && <p className="sync-alert">{create.error.message}</p>}</Dialog>}
    {action && <Dialog title={`${displayTitle(action.action)} certificate`} onClose={() => setAction(null)}><p>{action.request.subject.firstName} {action.request.subject.lastName} · {displayTitle(action.request.requestType)}</p>{action.action === "override" && <label>Target status<select value={action.targetStatus} onChange={event => setAction(previous => previous ? { ...previous, targetStatus: event.target.value as ActionDialog["targetStatus"] } : previous)}><option>APPROVED</option><option>PUBLISHED</option><option>REJECTED</option><option>REVOKED</option></select></label>}<label>Reason{action.reasonRequired ? " (required)" : " (optional)"}<textarea autoFocus maxLength={2000} value={reason} onChange={event => setReason(event.target.value)} /></label>{requiresStepUp && action.action === "override" && <><p className="sync-alert"><ShieldCheck size={16} /> Super Administrator overrides require recent authentication.</p>{session.authProvider === "local" ? <label>Current password<input type="password" autoComplete="current-password" value={password} onChange={event => setPassword(event.target.value)} /></label> : <button onClick={startMicrosoftStepUp}>Re-authenticate with Microsoft</button>}</>}<div className="modal-actions"><button onClick={() => setAction(null)}>Cancel</button><button className="primary" disabled={transition.isPending || (action.reasonRequired && reason.trim().length < 3) || (requiresStepUp && action.action === "override" && session.authProvider === "local" && !password)} onClick={() => transition.mutate(action)}>Confirm</button></div>{transition.isError && <p className="sync-alert">{transition.error.message}</p>}</Dialog>}
  </section>;
}
