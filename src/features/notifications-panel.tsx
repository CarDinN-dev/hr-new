...
type NotificationResponse = { data: Notification[]; meta: { unread: number } };
...
export function NotificationsPanel({ session, notify }: { session: BackendSession; notify: (message: string) => void }) {
  ...
  const query = useQuery({ queryKey: workflowKey(session, "notifications"), queryFn: () => apiRequest<NotificationResponse>("/notifications?limit=30"), enabled: canRead, refetchInterval: open ? 30_000 : 60_000 });
