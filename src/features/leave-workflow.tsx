  ...
  return <section className="stack">
    {(Boolean(session.employeeId) || canSubmitForEmployee) && hasPermission(session, "leave.self.create") && <>
      <div className="panel"><div className="panel-head"><div><h3>Leave balance</h3><span>{targetEmployeeId ? `${balanceYear} balances for the selected employee` : "Select an employee to view balances"}</span></div></div>
        {balances.isPending && targetEmployeeId ? <p className="muted">Loading leave balances…</p> : balances.isError ? <p className="sync-alert">{balances.error.message}</p> : <div className="recruitment-pipeline">{balances.data?.map(balance => <article className="candidate-card" key={balance.leaveType.id}><div><strong>{balance.leaveType.name}</strong><span>{balance.eligible ? "Current-year balance" : "Not eligible"}</span></div>{balance.noBalanceRequired ? <p>No balance required</p> : balance.eligible ? <div className="employee-card-details"><span><b>Total</b>{days(balance.totalDays)}</span><span><b>Used</b>{days(balance.usedDays)}</span><span><b>Pending</b>{days(balance.pendingDays)}</span><span><b>Available</b>{days(balance.availableDays)}</span></div> : <p>Unavailable</p>}</article>)}{targetEmployeeId && !balances.data?.length && <div className="empty compact">No active leave types.</div>}</div>}
      </div>
