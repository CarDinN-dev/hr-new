  ...
  return <section className="stack leave-workflow">
    ...
    <div className="panel leave-requests-panel"><div className="panel-head"><div><h3>Leave requests</h3><span>{all.length} {all.length === 1 ? "request" : "requests"} · {inbox.data?.length ?? 0} awaiting your decision</span></div></div>{records.isPending || (canInbox && inbox.isPending) ? <p className="muted">Loading leave requests…</p> : records.isError ? <p className="sync-alert">{records.error.message}</p> : <div className="table-wrap table-wide table-actions" role="region" aria-label="Leave requests"><table><thead><tr><th>Employee</th><th>Leave</th><th>Dates</th><th>Paid / unpaid</th><th>Attachment</th><th>Status</th><th>Actions</th></tr></thead><tbody>{all.map(request => {
      ...
      return <tr id={`leave-${request.id}`} tabIndex={-1} className="leave-request-row" key={request.id}>
        ...
        <td data-label="Status"><div className="leave-request-status"><span className="badge neutral">{displayTitle(request.status)}</span>{request.currentStage && <small>{displayTitle(request.currentStage)} stage</small>}</div></td>
