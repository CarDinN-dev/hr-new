...
export function LeaveWorkflowPage({ session, notify }: { session: BackendSession; notify: (message: string) => void }) {
  ...
  const submit = useMutation({
    mutationFn: () => apiRequest<LeaveRecord>("/leave/submit", { method: "POST", csrfToken: session.csrfToken, headers: idempotencyHeaders(), body: JSON.stringify({ ...form, employeeId: canSubmitForEmployee ? form.employeeId || undefined : undefined, leaveTypeId: form.leaveTypeId || leaveTypes.data?.[0]?.id }) }),
  ...
  const decide = useMutation({
    mutationFn: async (target: Decision) => {
      const requiresStepUp = target.action === "self-approve" && !isSuperAdmin;
      if (requiresStepUp && session.authProvider === "local") {
  ...
  return <section className="stack">
    ...
    <div className="panel"><div className="panel-head"><div><h3>Leave requests</h3><span>{inbox.data?.length ?? 0} assigned to you</span></div></div>{records.isPending || (canInbox && inbox.isPending) ? <p className="muted">Loading leave requests…</p> : records.isError ? <p className="sync-alert">{records.error.message}</p> : <div className="table-wrap table-wide table-actions" role="region" aria-label="Leave requests" tabIndex={0}><span className="table-scroll-hint" aria-hidden="true">Scroll horizontally for more columns</span><table><thead><tr><th>Employee</th><th>Leave</th><th>Dates</th><th>Status</th><th>Actions</th></tr></thead><tbody>{all.map(request => {
