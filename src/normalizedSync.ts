...
import { apiList, apiRequest, type BackendSession } from "./api";
...
export async function persistNormalizedStateDelta(before: HrState, after: HrState, session: BackendSession) {
  ...
  const [existingDepartments, existingLeaveTypes] = await Promise.all([
    apiList<BackendRecord>("/departments"),
    apiList<BackendRecord>("/leave/types")
  ]);
  await syncSettings(before, after, existingDepartments, existingLeaveTypes, request);
  ...
  await syncEmployees(before, after, departmentIds, request);
  await syncAttendance(before, after, request);
  await syncLeaves(before, after, leaveTypeIds, request, session);
  await syncTrips(before, after, request);
  await syncExpenses(before, after, request);
  await syncLoans(before, after, request);
  await syncRecruitment(before, after, departmentIds, request);
  await syncEos(before, after, request);
  await syncPayroll(before, after, request);
  await syncDocuments(before, after, session);
}

type RequestFn = <T>(path: string, method: string, body?: unknown) => Promise<T>;

async function syncEmployees(before: HrState, after: HrState, departmentIds: Map<string, string>, request: RequestFn) {
  ...
  for (const employee of after.employees) {
    ...
    if (!old) {
      ...
      await request(`/employees/${created.id}/details`, "PATCH", employeeDetailsPayload(employee));
    } else if (!same(old, employee)) {
  ...
  for (const employee of before.employees) if (!current.has(employee.id)) await request(`/employees/${employee.id}`, "DELETE");
}
...
function employeeDetailsPayload(employee: EmployeeRecord) {
  ...
  return {
    ...
    bankAccount: { bankCode: f["Bank Code"], iban: f["IBAN No."], accountNumber: f["Account No."] },
    benefits: {
    ...
    salary: {
      baseSalary: money(f.Basic), allowances: money(f.HRA) + money(f["Food Allowance"]) + money(f["Mobile Allowance"]) + money(f["Special Allowance"]),
      bonuses: money(f["Overtime Amount"]), deductions: 0, taxRate: 0
    },
    credentials: [
...
function employeePayload(employee: EmployeeRecord, departmentIds: Map<string, string>) {
  ...
  const gender = ({ Male: "MALE", Female: "FEMALE", Other: "OTHER" } as const)[f.Gender as "Male" | "Female" | "Other"];
  return compact({
    ...
    dateOfBirth: f["Date of Birth"], gender, hireDate: f["Joining Date"], employmentStatus: status,
    departmentId: departmentIds.get(f.Department), salary: money(f.Basic),
    emergencyContactName: f["Emergency Contact Name"], emergencyContactPhone: f["Emergency Contact Mobile No."]
  });
}
...
async function syncLeaves(before: HrState, after: HrState, typeIds: Map<string, string>, request: RequestFn, session: BackendSession) {
  ...
  for (const leave of after.leaves) {
    ...
    if (!old) {
      ...
      const hr = session.role === "SUPER_ADMIN" || session.role === "HR_ADMIN";
      await request("/leave/requests", "POST", { employeeId: hr ? leave.employeeId : undefined, leaveTypeId, startDate: leave.from, endDate: leave.to, isHalfDay: leave.days === 0.5, reason: leave.reason });
  ...
  for (const leave of before.leaves) if (!current.has(leave.id)) {
    ...
    if (session.role === "SUPER_ADMIN" || session.role === "HR_ADMIN") await request(`/leave/requests/${leave.id}`, "DELETE");
  }
}
...
async function syncRecruitment(before: HrState, after: HrState, departmentIds: Map<string, string>, request: RequestFn) {
  const oldJobs = byId(before.jobs);
  for (const job of after.jobs) if (!oldJobs.has(job.id)) await request("/recruitment/jobs", "POST", { title: job.title, departmentId: departmentIds.get(job.dept), openings: job.openings, postedOn: job.postedOn, description: job.description || undefined });
  ...
  for (const candidate of after.candidates) {
    ...
    if (!old) await request("/recruitment/candidates", "POST", { jobId: candidate.jobId, name: candidate.name, email: candidate.email, phone: candidate.phone || undefined, rating: String(candidate.rating), notes: candidate.notes || undefined, appliedOn: candidate.appliedOn });
    else if (old.stage !== candidate.stage) {
...
async function syncPayroll(before: HrState, after: HrState, request: RequestFn) {
  const previous = byId(before.payroll);
  for (const slip of after.payroll) {
    const old = previous.get(slip.id);
    if (!old) continue;
    if (old.status !== slip.status && slip.status === "Finalized") await request(`/payroll/${slip.id}/approve`, "POST");
    else if (!same(payrollAmounts(old), payrollAmounts(slip))) await request(`/payroll/${slip.id}`, "PATCH", payrollAmounts(slip));
  }
}
...
async function syncSettings(before: HrState, after: HrState, departments: BackendRecord[], leaveTypes: BackendRecord[], request: RequestFn) {
  if (!same(before.settings.company, after.settings.company) || before.settings.workdayHours !== after.settings.workdayHours || before.settings.halfDayHours !== after.settings.halfDayHours || !same(before.settings.loanDeductionCap, after.settings.loanDeductionCap)) {
